...
* **Estados del Modelo (`ST_SYS_NAME`):**
  - `ST_SYS_IDLE`: Estado de reposo o espera, el sistema aguarda la llegada de un vehículo o la interacción con el botón.
...
* **Eventos de Entrada / Disparadores (Triggers):**
  ...
  - `Tick` / `Timeout`: Disparadores temporizados para controlar el tiempo de apertura de la barrera.
...
| :--- | :--- | :--- | :--- | :--- |
| **ST_SYS_IDLE** | `EV_SYS_CAR_DETECTED` | | **ST_SYS_IDLE** | Habilitar interfaz de bienvenida en Display |
| **ST_SYS_IDLE** | `EV_SYS_BTN_DOWN` | | **ST_SYS_TICKET_PROCESSING** | `EV_ACT_PRINT_START`, Registrar en servidor |
| **ST_SYS_TICKET_PROCESSING** | `Tick` / Evento Impresión | `[Impresión Finalizada]` | **ST_SYS_BARRIER_OPEN** | `EV_ACT_BARRIER_UP`, `tick = DEL_BARRIER_TIMEOUT` |
